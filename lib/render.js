...
export default function render(tree, field, formatter) {
	...
	return run(tree.children, formatter, renderFields);
}

function run(nodes, formatter, renderFields) {
	return nodes.filter(notGroup).map(node => {
		const outNode = formatter(new OutputNode(node), renderFields);
		return outNode ? outNode.toString(run(node.children, formatter, renderFields)) : '';
	}).join('');
}
