 ...
 * `false` otherwise
 */
export function handlePseudoSnippet(outNode, renderFields) {
	...
	if (isPseudoSnippet(node)) {
		...
		if (field) {
            ...
            outNode.open = renderFields(parts[0]);
			outNode.close = renderFields(parts[1]);
		} else {
			outNode.text = renderFields(fieldsModel);
		}
